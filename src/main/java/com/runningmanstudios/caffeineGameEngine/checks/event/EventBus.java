...
public class EventBus implements Serializable {
    ...
    public boolean emit(Event event) {
        ...
        while (!foundEvent) {
            ...
            List<EventListener> superEventSubscribers = this.listeners.get(superClass);
            ...
            if (superEventSubscribers != null) {
                ...
                for (EventListener superEventSubscriber : superEventSubscribers) {
                    ...
                    if (!cancelled) {
                        try {
                            ...
                            System.out.println(superListenerMethod);
                            System.out.println(superUpper);
    ...
    public void subscribe(Object listener) {
        ...
        List<Method> subscribers = getEventBusSubscriberMethodsFrom(listenerClass);
        ...
        for (Method eventSubscriber : subscribers) {
            ...
            if (Event.class.isAssignableFrom(subscribingTo)) {
                ...
                if (!this.listeners.containsKey(subscribingTo)) {
                    this.listeners.put(subscribingTo, new CopyOnWriteArrayList<>());
                }
                this.listeners.get(subscribingTo).add(new EventListener(listener, eventSubscriber));
    ...
    /**
     * removes a classes' methods from the event bus, if the method has a single Event as a parameter and the method has the annotation @EventBusSubscriber.
     * if a class does not have any methods with the annotation @EventBusSubscriber, this will throw a EventException.
     * if a method has the annotation @EventBusSubscriber but the method does not have a single Event as a parameter, this will throw a EventException.
     * the parameter determines the Event this method will be associated with, e.g. if a method has "MoveEvent" as a parameter then every time MoveEvent is emitted the method will be invoked.
     * @param listener class to remove methods
     * @see EventBusSubscriber
     */
    public void unsubscribe(Object listener) {
        ...
        for (Method eventSubscriber : subscribers) {
            ...
            if (Event.class.isAssignableFrom(subscribingTo)) {
                ...
                this.listeners.get(subscribingTo).removeIf(el -> el.getUpperObject().equals(listener));
     ...
     */
    private static List<Method> getEventBusSubscriberMethodsFrom(final Class<?> type) {
    ...
    private class EventListener {
        ...
        public EventListener(Object upper, Method listener) {
            this.upper = upper;
